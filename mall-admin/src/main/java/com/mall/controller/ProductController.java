...
public class ProductController extends BaseController
{
	...
	public ModelAndView add()
	{
		...
		//ERP仓库列表
		//mv.addObject("warehouses", warehoues());
	...
	public String saveProduct(@RequestParam("id") Long productId,
			@RequestParam("productCategoryId") Long categoryId,
			@RequestParam("brandId") Long brandId)
	{
	...
	public ModelAndView editProduct(@RequestParam("id") Long productId)
	{
		...
		if (product != null)
		{
			...
			/*if(warehouse != null)
			{
				mv.addObject("cWarehouse", warehouse);
			}*/
  	...
  	private List<Map<String, Object> > getProductParamList(Product product)
	{
		...
		List<ParameterGroup> groupList = this.groupService.getList("productCategory", product.getProductCategory(), "orders asc");
	...
	private void saveProductParameters(Product product)
	{
		...
		if (params != null) 
		{
			...
			{
				ProductParameterValue parameterValue = this.productParamService.get(product.getId(), Long.valueOf(paramIds[i]));
	 ...
	 * @return
	 */
	private Map<String, String> warehoues()
	{
		Map<String, String> warehouseMap = new HashMap<>();
		/*for(Warehouse house : Warehouse.values())
		{
			warehouseMap.put(house.toString(), house.getName());
		}*/
		return warehouseMap;
	}
	 ...
	 * 产品规格查询接口
	 * 请求参数: id
	 * 返回数据: {id: xx, name: xx, values:[{id: xx, name: xx}, ...]}
	 * @param response
	 */
