...
<head>
<meta charset="UTF-8">
<title>售后申请处理通知-${config.siteName}</title>
<style type="text/css">
body{
	padding:0px;
	margin:0px;
...
<body>
	<div class="main">
        ...
        <div class="content">
	        <h4>尊敬的管理员您好：</h4>
            <p>${config.siteName}（<a href="${config.siteUrl}/admin/" target="_blank" >${config.siteUrl}</a>收到售后付款,请继续维修操作. 详情请登录${config.siteName}后台管理系统:</p>
            <p><a href="${config.siteUrl}/admin" target="_blank">${config.siteUrl}/admin/login.do</a></p>
            
            <#--订单详情-->
            <p>
            	申请单编号:${apply.sn}<br />
            	申请时间:${apply.createDate?datetime}<br />
            	付款时间: ${apply.repairOrder.payment.createDate?datetime}<br />
            </p>
