...
<body>
	<div class="path">
		<a href="${base}/admin/home.do">首页</a> &raquo; 换货单列表 <span>(共<span id="pageTotal">${pager.totalCount?c}</span>条记录)</span>
	</div>
	...
	<input type="hidden" id="status" name="status" <#if status??>value="${status}" </#if>/>
		...
		<table id="listTable" class="list">
			<tr>
				...
				<th>
					<a href="javascript:;" class="sort">是否审核过</a>
				</th>
				<th>
					<span>操作</span>
				</th>
		...
		<#list switchs as switch>
			<tr>
				<td>
					<input type="checkbox" name="ids" value="${switch.id}"/>
				</td>
				<td>${switch.sn}</td>
				<td>${switch.member.username}</td>
				<td> 
				 ${switch.statusView}
				</td>
				<td><#if switch.reason?length gt 20>${switch.reason[0..20]}...<#else>${switch.reason!''}</#if></td>
				<td>${switch.createDate}</td>
				<td><#if switch.checked>是<#else>否</#if></td>
				<td>
					<a href="${base}/admin/switch/view!${switch.id}.do">[查看]</a>
				</td>
			</tr>
		</#list>
		</table>
