...
<body>
	<div class="path">
		<a href="${base}/admin/home.do">首页</a> &raquo; 退货单列表 <span>(共<span id="pageTotal">${pager.totalCount?c}</span>条记录)</span>
	</div>
	...
	<input type="hidden" id="status" name="status" <#if status??>value="${status}" </#if>/>
		...
		<#list returns as return>
			<tr>
				<td>				
					<input type="checkbox" name="ids" value="${return.id}" />
				</td>
				<td>${return.sn}</td>
				<td>${return.member.username}</td>
				<td>${return.statusView}</td>
				<td><#if return.reason?length gt 20>${return.reason[0..20]}...<#else>${return.reason!''}</#if></td>
				<td>${return.createDate}</td>
				<td>
					<a href="${base}/admin/return/view!${return.id}.do">[查看]</a>
				</td>
			</tr>
		</#list>
		</table>
