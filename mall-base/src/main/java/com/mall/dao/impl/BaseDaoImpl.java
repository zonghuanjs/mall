...
@Repository
@Transactional
public abstract class BaseDaoImpl<PK extends Serializable, T> extends HibernateDaoSupport implements BaseDao<PK, T>
{
	...
	public List<T> get(PK[] ids)
	{
		...
		try
		{
			...
			Set<PK> objs = new HashSet<PK>();
			Collections.addAll(objs, ids);
			query.add(Restrictions.in(idColName(), objs));
			
			ScrollableResults results = query.scroll();
			while(results.next())
			{
				T obj = (T)results.get(0);
				list.add(obj);
			}
		}
		catch (Exception e)
